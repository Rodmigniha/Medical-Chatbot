...
def answer_quest(question, enable_insert=True):
    ...
    try:
        ...
        scores = [float(score) for doc, score in docs_with_scores]  # Convertir les scores en float (éviter numpy.float32)
        ...
        # Gestion des erreurs (ajouter des logs si nécessaire)
        print(f"Erreur dans la génération de la réponse : {e}")
        return None, None, None
